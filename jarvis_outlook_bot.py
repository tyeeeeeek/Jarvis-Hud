...
#   default (same person, same phone) unless JARVIS_OUTLOOK_CHAT_ID
#   overrides it.
#
#   Fully inert until JARVIS_OUTLOOK_BOT_TOKEN is set in .env.
# ================================================================
import os
import time
...
BOT_TOKEN = os.environ.get("JARVIS_OUTLOOK_BOT_TOKEN", "").strip()
CHAT_ID = os.environ.get("JARVIS_OUTLOOK_CHAT_ID", "").strip() or os.environ.get("TELEGRAM_CHAT_ID", "").strip()
API_BASE = f"https://api.telegram.org/bot{BOT_TOKEN}" if BOT_TOKEN else ""

JARVIS_OUTLOOK_AVAILABLE = bool(BOT_TOKEN and CHAT_ID)

_AGENT_NAME = "JarvisOutlook"
...
def poll_thread(on_command, pipeline_stop):
    ...
    if not JARVIS_OUTLOOK_AVAILABLE:
        print("  [JarvisOutlook] Not configured -- texting the email bot is disabled. Set JARVIS_OUTLOOK_BOT_TOKEN in .env.")
        return

    print(f"  [JarvisOutlook] Watching for messages from chat {CHAT_ID}")
    ...
    try:
        ...
        print(f"  [JarvisOutlook] Startup sync error: {e}")
    ...
    while not pipeline_stop.is_set():
        try:
            ...
            for update in updates:
                ...
                if not text:
                    continue
                print(f"  [JarvisOutlook] Received: {text!r}")
                try:
                    on_command(text)
                except Exception as e:
                    print(f"  [JarvisOutlook] Command handling error: {e}")
                    send_message("Something went wrong handling that sir.")
        except requests.Timeout:
            continue
        except Exception as e:
            print(f"  [JarvisOutlook] Poll error: {e}")
            time.sleep(5)
